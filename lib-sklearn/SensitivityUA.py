...
def uncertainty(func, paramrefs, paramuncerts, paramuncerttypes, diff_or_fact, lin_or_power):
    [refval,sensitivities,linearities] = sensitivity(func, paramrefs, paramuncerts, diff_or_fact, lin_or_power)
    uncerts=[]
    totaluncert=0.0
    totaluncertdB=0.0
    if (len(paramrefs)!=len(paramuncerts)) or (len(paramrefs)!=len(paramuncerttypes)):
    ...
    for i in range(len(paramrefs)):
        if lin_or_power:
            if diff_or_fact:
                ...
                uncerts.append(sensitivities[i]*paramrefs[i]*(paramuncerts[i]-1)) #not symmetric
        else:
            if diff_or_fact:
                uncerts.append(sensitivities[i]*np.log(paramuncerts[i]/paramrefs[i]+1)) #not symmetric
            else:
...
u=uncertainty(myfunc, myparamrefs, myparamuncerts, myparamuncerttypes, diff_or_fact, lin_or_power)
...
def MetropolisHastingsUncertainty(func, paramrefs, paramuncerts, paramuncerttypes, initcount, totalcount): #diff_or_fact
    n=len(paramrefs)
    jumpfactor=0.5
    alpha=0.0
    valsum=0.0
    ...
    while counter<totalcount:
        ...
        if paramuncerttypes[i]=='R':
            alpha=uniform.pdf(candidate,paramrefs[i]-paramuncerts[i],2*paramuncerts[i])/uniform.pdf(currentparams[i],paramrefs[i]-paramuncerts[i],2*paramuncerts[i])
        else:
            alpha=norm.pdf(candidate,paramrefs[i],paramuncerts[i])/norm.pdf(currentparams[i],paramrefs[i],paramuncerts[i])
...
for i in range(nrruns):
    estimates[i,:]=MetropolisHastingsUncertainty(myfunc, myparamrefs, myparamuncerts, myparamuncerttypes, 20, 1000)

print('(',estimates[:,0].mean(),'+-',estimates[:,0].std(),')','+-','(',estimates[:,1].mean(),'+-',estimates[:,1].std(),')')
...
for i in range(nrruns):
    estimates[i,:]=MetropolisHastingsUncertainty(myfunc, myparamrefs, myparamuncerts, myparamuncerttypes, 1000, 10000)
...
MetropolisHastingsUncertainty(myfunc, myparamrefs, myparamuncerts, myparamuncerttypes, 100, 100000)
